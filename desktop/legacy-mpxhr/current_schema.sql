...
CREATE TABLE attendance (
    ...
    FOREIGN KEY (employee_id) REFERENCES "employees_old"(id)
);
...
CREATE TABLE assets (
            ...
            assigned_date DATE,
            value REAL,
            FOREIGN KEY(assigned_to) REFERENCES "employees_old"(id)
        );

CREATE TABLE holidays (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, date DATE UNIQUE, type TEXT, company_id INTEGER);

CREATE TABLE leave_balances (id INTEGER PRIMARY KEY AUTOINCREMENT, employee_id INTEGER, leave_type TEXT, year INTEGER, balance REAL, used REAL DEFAULT 0, UNIQUE(employee_id, leave_type, year));
